...
def prepare_for_dn(dn_args, tgt_weight, embedweight, batch_size, training, num_queries, num_classes, hidden_dim, label_enc):
    ...
    tgt = torch.cat([tgt_weight, indicator0], dim=1) + label_enc.weight[0][0] * torch.zeros((), device=device)
    ...
    if training:
        ...
        if len(known_num):
            map_known_indice = torch.cat([torch.arange(num, device=device) for num in known_num])  # [1,2, 1,2,3]
...
def tgt_loss_boxes(src_boxes, tgt_boxes, num_tgt,):
    ...
    if len(tgt_boxes) == 0:
        return {
            'tgt_loss_bbox': src_boxes.sum() * 0.0,
            'tgt_loss_giou': src_boxes.sum() * 0.0,
        }
...
def tgt_loss_labels(src_logits_, tgt_labels_, num_tgt, focal_alpha, log=True):
    ...
    if len(tgt_labels_) == 0:
        return {
            'tgt_loss_ce': src_logits_.sum() * 0.0,
            'tgt_class_error': src_logits_.sum().detach() * 0.0,
        }
...
def compute_dn_loss(mask_dict, training, aux_num, focal_alpha, device=None):
    """
    ...
    losses = {}
    if device is None:
        if mask_dict is not None and 'device' in mask_dict:
            device = mask_dict['device']
        else:
    ...
    if training and mask_dict is not None and 'output_known_lbs_bboxes' in mask_dict:
        ...
        losses['tgt_loss_bbox'] = torch.zeros((), device=device)
        losses['tgt_loss_giou'] = torch.zeros((), device=device)
        losses['tgt_loss_ce'] = torch.zeros((), device=device)
        losses['tgt_class_error'] = torch.zeros((), device=device)
    ...
    if aux_num:
        for i in range(aux_num):
            ...
            if training and mask_dict is not None and 'output_known_lbs_bboxes' in mask_dict:
                ...
                l_dict = dict()
                l_dict['tgt_loss_bbox'] = torch.zeros((), device=device)
                l_dict['tgt_class_error'] = torch.zeros((), device=device)
                l_dict['tgt_loss_giou'] = torch.zeros((), device=device)
                l_dict['tgt_loss_ce'] = torch.zeros((), device=device)
                l_dict = {k + f'_{i}': v for k, v in l_dict.items()}
                losses.update(l_dict)
    return losses
