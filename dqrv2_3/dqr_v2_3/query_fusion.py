...
class NormConstrainedRegionGuidedDNFusion(nn.Module):
    """V2 fusion with a per-query cap on the final residual magnitude."""
    ...
    def forward(
        ...
        if len(active) == 0:
            ...
            return queries, {
                ...
                "bqr_gate_mean": zero,
                "bqr_query_norm": zero,
                "bqr_projected_norm": zero,
                "bqr_raw_delta_norm": zero,
                "bqr_fusion_delta_norm": zero,
                "bqr_raw_delta_ratio": zero,
                "bqr_final_delta_ratio": zero,
                "bqr_clip_rate": zero,
        ...
        with torch.autocast(device_type=queries.device.type, enabled=False):
            ...
            alpha = self.gate(
                torch.cat((active_queries, projected), dim=-1)
            ).sigmoid()

            raw_delta = alpha * projected
            epsilon = 1e-6
            query_norm = active_queries.norm(dim=-1, keepdim=True).clamp_min(epsilon)
            raw_delta_norm = raw_delta.norm(dim=-1, keepdim=True)
            max_delta_norm = self.residual_ratio * query_norm
            scale = torch.clamp(
                max_delta_norm / raw_delta_norm.clamp_min(epsilon),
                max=1.0,
            )
            delta = raw_delta * scale
            fused_active = active_queries + delta
        ...
        final_delta_norm = delta.detach().norm(dim=-1, keepdim=True)
        query_norm_detached = query_norm.detach()
        diagnostics = {
            "bqr_valid_queries": torch.as_tensor(
                float(len(active)), device=queries.device
            ),
            "bqr_gate_mean": alpha.detach().mean(),
            "bqr_query_norm": query_norm_detached.mean(),
            "bqr_projected_norm": projected.detach().norm(dim=-1).mean(),
            "bqr_raw_delta_norm": raw_delta_norm.detach().mean(),
            "bqr_fusion_delta_norm": final_delta_norm.mean(),
            "bqr_raw_delta_ratio": (
                raw_delta_norm.detach() / query_norm_detached
            ).mean(),
            "bqr_final_delta_ratio": (
                final_delta_norm / query_norm_detached
            ).mean(),
            "bqr_clip_rate": (scale.detach() < 0.999).float().mean(),
...
# Keep the familiar V2 class name available inside the isolated package.
RegionGuidedDNFusion = NormConstrainedRegionGuidedDNFusion
