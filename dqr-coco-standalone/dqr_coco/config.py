...
class ExperimentConfig:
    ...
    def __post_init__(self) -> None:
        ...
        if invalid:
            raise ValueError(
                f"Settings must be positive: {', '.join(invalid)}"
            )
        ...
        if self.diagnostics_every < 0:
            raise ValueError(
                "diagnostics_every must be non-negative"
            )
    ...
    def method_recipe(self) -> dict:
        ...
        if self.method == "bqr_dn_v2":
            recipe.update(
                {
                    "bqr_points_per_level":
                        self.bqr_points_per_level,
                    "bqr_gate_bias":
                        self.bqr_gate_bias,
                }
            )
    ...
    def resolved_run_name(self) -> str:
        return self.run_name or (
            f"coco_gb{self.target_global_batch_size}_"
            f"{self.precision}_"
            f"b{self.batch_size}"
            f"a{self.accumulation_steps}_"
            f"{self.comparison_fingerprint[:8]}"
        )
    ...
    def initialization_path(self) -> Path:
        return (
            ...
            / (
                f"dino_r50_4scale_"
                f"seed{self.seed}_"
                f"{self.detector_fingerprint}.pt"
            )
        )

    # ------------------------------------------------------------------
    # Distributed batch validation
    # ------------------------------------------------------------------
    ...
    def validate_world_size(
        ...
        if effective != self.target_global_batch_size:
            raise ValueError(
                ...
                f"{self.batch_size} "
                f"x {world_size} "
                f"x {self.accumulation_steps} "
                f"= {effective}, "
                f"expected "
                f"{self.target_global_batch_size}"
            )

    # ------------------------------------------------------------------
    # Official DINO argument construction
    # ------------------------------------------------------------------
    ...
    def official_args(
        ...
        overrides = {
            ...
            # Required by datasets/coco.py.
            # False preserves standard DINO multi-scale augmentation.
            "fix_size": False,
