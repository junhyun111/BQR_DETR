...
def resolve_resume_path(config: ExperimentConfig, resume: str | Path | None) -> Path | None:
    if resume is None:
        return None
...
def load_training_state(
    ...
    if checkpoint.get("upstream_commit") != upstream_commit():
        raise ValueError("Checkpoint official DINO commit mismatch")
    if checkpoint.get("method") != config.method:
        raise ValueError("Checkpoint method mismatch")
    if checkpoint.get("comparison_fingerprint") != config.comparison_fingerprint:
        raise ValueError("Checkpoint comparison recipe mismatch")
    if checkpoint.get("detector_fingerprint") != config.detector_fingerprint:
        raise ValueError("Checkpoint detector recipe mismatch")
    ...
    if (
        ...
        raise ValueError("Checkpoint common detector initialization mismatch")
