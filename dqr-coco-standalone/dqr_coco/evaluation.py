...
def evaluate(
    ...
    images = torch.tensor(float(local_images), device=context.device)
    all_reduce_sum(images, context)
    stats = evaluator.coco_eval["bbox"].stats
    metrics = {name: float(stats[index]) for index, name in enumerate(COCO_STAT_NAMES)}
    metrics["val_time_seconds"] = float(elapsed.item())
    metrics["val_images"] = int(images.item())
    return metrics
