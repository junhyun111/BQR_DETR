...
class SetCriterion(nn.Module):
    ...
    def forward(self, outputs, targets, return_indices=False, num_boxes_override=None):
        ...
        # Compute the average number of target boxes across all nodes.  A
        # caller performing gradient accumulation may supply the average over
        # the complete virtual-rank window so 2 GPUs x 4 micro-steps has the
        # same loss normalization as an 8-rank DINO update.
        if num_boxes_override is None:
            ...
            if torch.is_tensor(num_boxes_override):
                num_boxes = torch.clamp(
                    num_boxes_override.to(device=device, dtype=torch.float32), min=1
                )
            else:
        ...
        if self.training and dn_meta and 'output_known_lbs_bboxes' in dn_meta:
            ...
            l_dict = dict()
            l_dict['loss_bbox_dn'] = torch.as_tensor(0., device=device)
            l_dict['loss_giou_dn'] = torch.as_tensor(0., device=device)
            l_dict['loss_ce_dn'] = torch.as_tensor(0., device=device)
            l_dict['loss_xy_dn'] = torch.as_tensor(0., device=device)
            l_dict['loss_hw_dn'] = torch.as_tensor(0., device=device)
            l_dict['cardinality_error_dn'] = torch.as_tensor(0., device=device)
            losses.update(l_dict)
