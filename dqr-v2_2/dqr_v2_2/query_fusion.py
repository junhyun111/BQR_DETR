...
class RegionGuidedDNFusion(nn.Module):
    """V2 region fusion with a configured residual-correction multiplier."""
    ...
    def forward(
        ...
        with torch.autocast(device_type=queries.device.type, enabled=False):
            ...
            fused_active = (
                active_queries + self.fusion_weight * alpha * projected
            )
        ...
        diagnostics = {
            ...
            "bqr_effective_gate_mean": (
                self.fusion_weight * alpha.detach()
            ).mean(),
            ...
            "bqr_fusion_delta_norm": (
                fused_active.detach() - active_queries.detach()
            ).norm(dim=-1).mean(),
        }
